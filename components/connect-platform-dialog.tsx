...
const platforms = [
    ...
    { id: 'tiktok', label: 'TikTok', icon: Smartphone, color: 'text-[#000000] dark:text-zinc-100', activeBg: 'bg-muted', description: 'TikTok Ads Manager' },
    ...
    { id: 'x', label: 'X (Twitter)', icon: Twitter, color: 'text-[#000000] dark:text-zinc-100', activeBg: 'bg-muted', description: 'X Ads Center' },
...
export function ConnectPlatformDialog({ open, onOpenChange, connectedPlatforms, enabledPlatforms, onConnect, onDisconnect }: ConnectPlatformDialogProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[480px] p-0 overflow-hidden rounded-[28px] border-border shadow-3xl bg-card">
                <div className="p-8 pb-4">
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-10 h-10 rounded-md bg-blue-100 dark:bg-blue-500/10 flex items-center justify-center">
                            <Plus className="w-5 h-5 text-primary" />
                        </div>
                        <div>
                            <DialogTitle className="text-xl font-semibold tracking-tight">Connect New Platform</DialogTitle>
                        ...
                        {filteredPlatforms.map((platform) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        "flex items-center gap-4 p-4 rounded-md border transition-all duration-300 text-left group",
                                        isMeta ? "bg-muted/20 border-zinc-100 dark:border-zinc-700/30 cursor-default" : "bg-muted/40 border-zinc-100 dark:border-zinc-700/50 hover:border-primary/30 hover:bg-white dark:hover:bg-zinc-800 hover:shadow-lg cursor-pointer",
                                ...
                                >
                                    <div className={cn(
                                        ...
                                        "bg-card border border-zinc-100 dark:border-zinc-700 shadow-sm group-hover:shadow-md",
                                        platform.color
                                    )}>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            <h4 className="text-sm font-black text-foreground">{platform.label}</h4>
                                            {(isConnected || isMeta) && <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />}
                                        </div>
                                        <p className="text-[10px] text-muted-foreground font-medium line-clamp-1">{platform.description}</p>
                                    </div>

                                    {isMeta ? (
                                        <div className="text-[10px] font-medium uppercase tracking-wider px-2.5 py-1 rounded-full bg-zinc-100 dark:bg-zinc-700/50 text-muted-foreground">
                                            Core Platform
                                        </div>
                                        ...
                                        <div className={cn(
                                            "transition-opacity text-xs font-medium uppercase tracking-wider px-3 py-1.5 rounded-xl shadow-sm cursor-pointer active:scale-95",
                ...
                <div className="p-6 bg-muted/20 border-t border-zinc-100 dark:border-border flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <Sparkles className="w-3.5 h-3.5 text-primary animate-pulse" />
                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Enterprise Sync v2</span>
                    </div>
                    <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => onOpenChange(false)}
                        className="text-xs font-medium uppercase tracking-wider text-muted-foreground hover:text-zinc-900"
                    >
                        Maybe Later
                    </Button>
                </div>
