...
// ══════════════════════════════════════════════════════════════
async function fetchBaseAds(dateRange, campaignId) {
...
//  STEP 2: RSA QUERY
// ══════════════════════════════════════════════════════════════
async function enrichRSA(adMap, dateRange, campaignId) {
...
// ══════════════════════════════════════════════════════════════
async function enrichDisplay(adMap, dateRange, campaignId) {
...
// ══════════════════════════════════════════════════════════════
async function enrichDemandGen(adMap, dateRange, campaignId) {
...
//  STEP 5: VIDEO ADS
// ══════════════════════════════════════════════════════════════
async function enrichVideoAds(adMap, dateRange, campaignId) {
...
// ══════════════════════════════════════════════════════════════
async function enrichFromAssetView(adMap, campaignId) {
...
// ══════════════════════════════════════════════════════════════
async function fetchPMAX(dateRange, campaignId) {
    ...
    const rows = await safeQuery('PMAX Asset Groups', `
    ...
    const assetRows = await safeQuery('PMAX Assets', `
    ...
    try {
        const r1 = await queryGoogleAds(`SELECT asset.id, metrics.impressions, metrics.clicks, metrics.cost_micros, metrics.conversions, metrics.all_conversions_value FROM campaign_asset WHERE campaign.advertising_channel_type = 'PERFORMANCE_MAX' AND segments.date DURING ${dateRange} LIMIT 50000`);
        r1.forEach(r => merge(r.asset?.id, r.metrics || {}, 'campaign_asset'));
    } catch { }
    try {
        const r2 = await queryGoogleAds(`SELECT asset.id, metrics.impressions, metrics.clicks, metrics.cost_micros, metrics.conversions, metrics.all_conversions_value FROM asset_group_asset WHERE campaign.advertising_channel_type = 'PERFORMANCE_MAX' AND segments.date DURING ${dateRange} LIMIT 50000`);
        r2.forEach(r => merge(r.asset?.id, r.metrics || {}, 'asset_group_asset'));
    } catch { }
...
//  MAIN: fetchAds
// ══════════════════════════════════════════════════════════════
export async function fetchAds(dateRange = 'LAST_30_DAYS', campaignId = null) {
    const adMap = await fetchBaseAds(dateRange, campaignId);
    await Promise.allSettled([
        enrichRSA(adMap, dateRange, campaignId),
        enrichDisplay(adMap, dateRange, campaignId),
        enrichDemandGen(adMap, dateRange, campaignId),
        enrichVideoAds(adMap, dateRange, campaignId)
    ]);
    await enrichFromAssetView(adMap, campaignId);
    const pmaxEntries = await fetchPMAX(dateRange, campaignId);
...
export async function fetchAdAssets(adId, dateRange = 'LAST_30_DAYS') {
    const ads = await fetchAds(dateRange);
